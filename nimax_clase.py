...
class NISensorDAQ():
    ...
    def get_freq(self):
        return self.task.timing.samp_clk_rate, "Hz"
    
    def set_freq(self, f):
        self.task.timing.samp_clk_rate = f
    ...
    def set_chan(self, device, chan, config): #Asigns device, channel and acquisition config (DIFF, NRSE, RSE) to task
        if config == 'DIFF':
            self.task.ai_channels.add_ai_voltage_chan("Dev" + str(device) + "/" + str(chan)), 
                                         terminal_config = ni.constants.TerminalConfiguration.DIFFERENTIAL)
        elif config == 'NRSE':
            self.task.ai_channels.add_ai_voltage_chan("Dev" + str(device) + "/" + str(chan)), 
                                         terminal_config = ni.constants.TerminalConfiguration.NRSE)
        elif config == 'RSE':
            self.task.ai_channels.add_ai_voltage_chan("Dev" + str(device) + "/" + str(chan)), 
    ...
    def measure(self, points=1024, plot=False): #Measures a set amount of consecutive voltage points
        ...
        if plot:
            pp.plot(np.linspace(0, points/self.task.timing.samp_clk_rate, points), data)
        return data
    
    def reset_task: #Re-creates the task
        self.task = ni.Task()
